...
function s(D) {
  ...
  for (m = 0; m < 8; m++) {
    var E = 0, C = 0;
    E = D[m * 6 + 0] * 2 + D[m * 6 + 5];
    C = D[m * 6 + 1] * 2 * 2 * 2 + D[m * 6 + 2] * 2 * 2 + D[m * 6 + 3] * 2 +
        D[m * 6 + 4];
    switch (m) {
      case 0:
        F = A(N[E][C]);
...
// var g = {strEncSimple: o, strDecSimple: h, strEnc: d, strDec: p};
  return d(X, '1', '2', '3');
}